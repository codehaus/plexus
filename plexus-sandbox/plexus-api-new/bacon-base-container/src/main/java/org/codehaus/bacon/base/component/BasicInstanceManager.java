...
public class BasicInstanceManager
{
    ...
    private final Map injectorsByKey;

    public BasicInstanceManager( Map instanceFactoriesByLanguage, Map injectorsByKey, Container container,
                                ClassLoader containerLoader )
    {
        this.instanceFactoriesByLanguage = instanceFactoriesByLanguage;
        this.injectorsByKey = injectorsByKey;
        ...
        throws InstantiationException
    {
        ...
        if ( instance == null )
        {
            ...
            // fail fast.
            if ( !instanceFactoriesByLanguage.containsKey( lang ) )
            {
            ...
            InstanceFactory factory = (InstanceFactory) instanceFactoriesByLanguage.get( lang );

            instance = factory.instantiate( descriptor, containerLoader );

            compose( instance, descriptor, sessionKey, context );
    ...
    private void compose( Object instance, ComponentDescriptor descriptor, SessionKey sessionKey, Map context )
        throws InstantiationException
    {
        injectNonConstructorAttributes( instance, descriptor, sessionKey, context );
        injectNonConstructorReferences( instance, descriptor, sessionKey, context );
    }
        ...
        throws InstantiationException
    {
        ...
        for ( Iterator it = references.iterator(); it.hasNext(); )
        {
            ...
            String injectionStrategy = reference.getInjectionStrategy();

            // TODO Inject the reference.
        }
    }
        ...
        throws InstantiationException
    {
        ...
        for ( Iterator it = constructionRequirements.iterator(); it.hasNext(); )
        {
            ...
            if ( attribute != null )
            {
                ...
                {
                    throw new InstantiationException( "Error evaluating attribute expression for: \'" + id
                        + "\'. Class: " + type + " could not be found." );
                }
