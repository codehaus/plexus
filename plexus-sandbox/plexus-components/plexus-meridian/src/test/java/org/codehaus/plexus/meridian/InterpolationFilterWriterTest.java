    ...
    extends TestCase
{
        ...
        throws Exception
    {
        ...
        w.write( "My name is ${name} ${name} ${name} ${name}" );

        System.out.println( "result = " + sw.toString() );

        assertEquals( "My name is jason jason jason jason", sw.toString() );
    }
    ...
    protected InterpolationHandler createMap()
    {
        Map m = new HashMap();

        m.put( "name", "jason" );

        InterpolationHandler h = new MapInterpolationHandler( m );
