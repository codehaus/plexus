 ...
 * @version 1.0
 */
public class DeferrableTransitable
    implements Deferrable, Transitable
{
    ...
    public DeferrableTransitable( Packet packet )
    {
        ...
        while ( iAttributes.hasNext() )
        {
            ...
            if ( name.equals( "to" ) )
            {
                setTo( (String) attributes.get( name ) );
            }
            else if ( name.equals( "from" ) )
            {
                setFrom( (String) attributes.get( name ) );
            }
            else if ( name.equals( "type" ) )
            {
                setType( (String) attributes.get( name ) );
            }
            else
            {
                m_attributes += " " + name + "='" + (String) attributes.get( name ) + "'";
            }
        }
