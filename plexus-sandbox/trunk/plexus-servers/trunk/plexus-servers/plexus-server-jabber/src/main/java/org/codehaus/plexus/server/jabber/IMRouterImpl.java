...
import org.codehaus.plexus.server.jabber.data.storage.DeferrableListRepositoryHolder;
import org.codehaus.plexus.server.jabber.data.Transitable;
import org.codehaus.plexus.server.jabber.data.Deferrable;
import org.codehaus.plexus.server.jabber.data.jabber.DeferrableTransitable;
import org.codehaus.plexus.server.jabber.data.jabber.User;
import org.codehaus.plexus.server.jabber.tools.JIDParser;

import org.codehaus.plexus.server.jabber.log.MessageLogger;
import org.codehaus.plexus.server.jabber.log.MessageRecorder;
 ...
 * @avalon.service type="org.codehaus.plexus.server.jabber.IMRouter"
 */
    ...
    implements IMRouter, Initializable, Configurable, Serviceable
{
     ...
     * @avalon.dependency type="org.codehaus.plexus.server.jabber.data.storage.DeferrableListRepositoryHolder:1.0"  key="DeferrableListRepositoryHolder"
     ...
     * @avalon.dependency type="org.codehaus.plexus.server.jabber.log.MessageLogger:1.0"  key="MessageLogger"
     * @avalon.dependency type="org.codehaus.plexus.server.jabber.log.MessageRecorder:1.0"  key="MessageRecorder"
     */
    public void service( ServiceManager serviceManager )
        throws org.apache.avalon.framework.service.ServiceException
    {
        m_serviceManager = serviceManager;
        m_deferrableListHolder =
            (DeferrableListRepositoryHolder) serviceManager.lookup( "DeferrableListRepositoryHolder" );
        ...
        throws ConfigurationException
    {
        m_deliveryMessageQueueTimeout =
            configuration.getChild( "delivery-message-queue-timeout" ).getValueAsLong( 1000 * 60 * 60 );
        ...
        getLogger().info(
            "Router having delivery max retry: " + m_deliveryMaxRetry + " and delay " + m_deliveryRetryDelay );
    }
    ...
    public void registerSession( final IMClientSession session )
    {
        ...
        if ( session.getConnectionType() == IMSession.C2S_CONNECTION && user != null )
        {
            ...
            getLogger().debug(
                "Register session user: " + user.getNameAndRessource() + " session id " + session.getId() );
            try
            {
    ...
    public void unregisterSession( final IMClientSession session )
    {
        if ( session instanceof IMClientSession )
        {
            ...
            if ( user != null )
            {
                getLogger().debug( "Unregister register session user: " + user.getJIDAndRessource() + " session id " +
                    session.getId() );
    ...
    //-------------------------------------------------------------------------
    public void route( final IMSession currentSession,
                       final Transitable transit )
        throws java.io.IOException
    {
        ...
        if ( m_serverParameters.getHostNameList().contains( toHostname ) )
        { // local delivery
    ...
    //-------------------------------------------------------------------------
    private void enqueueRemoteDelivery( Transitable transitable,
                                        IMSession session )
    {
        ...
        {
            RemoteDeliveryThreadPerHost remoteDeliveryThread =
                (RemoteDeliveryThreadPerHost) m_remoteDeliveryThreadMap.get( hostname );
    ...
    public class TransitableAndSession
    {
        ...
        public TransitableAndSession( Transitable transitable,
                                      IMSession session )
        {
        ...
        extends Thread
    {
        ...
        public void enqueue( TransitableAndSession tas )
        {
            ...
            {
                getLogger().debug( "Adding tas for " + m_hostname + " this thread (" + this + ") isAlive: " +
                    isAlive() + " current status: " + m_currentStatus );
            ...
            while ( true )
            {
                ...
                synchronized ( this )
                {
                    if ( m_perHostRemoteDeliveryQueue.isEmpty() )
                    {
                        ...
                        {
                            getLogger().debug(
                                "Thread (" + this + "/" + m_hostname + ") wait " + m_deliveryMessageQueueTimeout );
                ...
                synchronized ( m_remoteDeliveryThreadMap )
                {
                    if ( m_perHostRemoteDeliveryQueue.isEmpty() )
                    {
                        ...
                        RemoteDeliveryThreadPerHost remoteDeliveryThread =
                            (RemoteDeliveryThreadPerHost) m_remoteDeliveryThreadMap.remove( m_hostname );
        ...
        private void deliver( TransitableAndSession tas )
        {
            ...
            try
            {
                ...
                for ( int retry = 0; retry < m_deliveryMaxRetry; retry++ )
                {
                    ...
                    {
                        getLogger().debug(
                            "Trying to send (" + transitable + ") to hostname " + m_hostname + " step " + retry );
                        if ( m_remoteSession == null || m_remoteSession.isClosed() )
                        {
                            m_remoteSession = m_s2sConnectorManager.getRemoteSessionWaitForValidation( m_hostname,
                                                                                                       m_deliveryMessageQueueTimeout );
                        }
