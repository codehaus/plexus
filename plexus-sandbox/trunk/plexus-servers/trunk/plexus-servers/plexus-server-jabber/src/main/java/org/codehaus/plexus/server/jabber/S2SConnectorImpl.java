    ...
    implements S2SConnector, Runnable, Configurable, Serviceable
{
    ...
    public void run()
    {
        ...
        try
        {
            ...
            getLogger().debug( "Trying to connect (timeout " + m_deliveryConnectionTimout + " ms) to " + m_toHostName +
                ":" + m_serverParameters.getRemoteServerPort() );
            socket.connect( insa, m_deliveryConnectionTimout );
            getLogger().info(
                "Connection to " + m_toHostName + ":" + m_serverParameters.getRemoteServerPort() + " successfull" );
            ...
            s += "<stream:stream xmlns:stream='http://etherx.jabber.org/streams' " + "xmlns='jabber:server' " + "to='" +
                m_toHostName + "' " + "from='" + m_serverParameters.getHostName() + "' " + "id='" + session.getId() +
        ...
        throws IOException
    {
        ...
        else
        {
            ...
            if ( m_session.getDialbackValue() == null )
            {
                ...
                String s = "<db:result from='" + m_serverParameters.getHostName() + "' to='" + m_toHostName + "'>";
                s += dialbackValue;
                s += "</db:result>";
        ...
        throws IOException
    {
        ...
        {
            String s =
                "<db:verify from='" + m_serverParameters.getHostName() + "' to='" + m_toHostName + "' id='" + id + "'>";
            s += dialbackValue;
            s += "</db:verify>";
            m_session.writeOutputStream( s );
        }
