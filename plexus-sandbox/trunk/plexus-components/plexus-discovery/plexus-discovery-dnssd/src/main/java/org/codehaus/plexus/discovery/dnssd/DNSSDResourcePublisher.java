 ...
 *
 * @author Aldrin Leal
 * @plexus.component role="org.codehaus.plexus.discovery.ResourcePublisher" lifecycle-handler="basic"
 */
public class DNSSDResourcePublisher
    implements ResourcePublisher, LogEnabled, Startable
{
     ...
     *
     * @param keyData keyData being used
     */
     ...
     *
     * @param domain the DNS Domain, like 'services.modafocas.org.'
     */
     ...
     *
     * @param disableTcp true if TCP should be disabled
     */
     ...
     *
     * @param keyAlgorithm keyData Algorithm, like "hmac-md5"
     */
     ...
     *
     * @param defaultTtl TTL Value, in seconds
     */
     ...
     * @throws org.codehaus.plexus.discovery.CodecException
     *          Someone set us up the bomb
            ...
            for ( int i = 0; i < nvps.length; i++ )
            {
                ...
                if ( -1 != ndx )
                {
                    ...
                    else
                    {
                         ...
                         */
                        hashTable.put( nvp.substring( 0, ndx ), "" );
                    }
                }
                else
                {
     ...
     *
     * @param resource resource to get registered
        ...
        throws ResourceRegistrationException
    {
        try
        {
            ...
            if ( DNSSDDiscoverableResource.class.isAssignableFrom( resource.getClass() ) )
            {
                dnssdr = (DNSSDDiscoverableResource) resource;

                if ( dnssdr.getPtrRecord() != null || dnssdr.getSrvRecord() != null || dnssdr.getTxtRecord() != null )
                {
                    throw new ResourceRegistrationException( "Resource already registered!" );
                }
            }
            else
            {
                throw new ResourceRegistrationException(
                    "Resource must be of class " + DNSSDDiscoverableResource.class.getCanonicalName() );
            }

            Name domainName = Name.fromString( this.getDomain() );
            Name serviceDomainName = Name.fromString( dnssdr.getType(), domainName );
            Name fullName = Name.fromString( dnssdr.getName(), serviceDomainName );

            PTRRecord ptrRecord = new PTRRecord( serviceDomainName, DClass.IN, this.defaultTtl, fullName );
            SRVRecord srvRecord = new SRVRecord( fullName, DClass.IN, this.defaultTtl, 0, 0, dnssdr.getUrl().getPort(),
                                                 Name.fromString( dnssdr.getUrl().getHost() ) );
            TXTRecord txtRecord =
                new TXTRecord( fullName, DClass.IN, this.defaultTtl, "path=" + dnssdr.getUrl().getPath() );
            ...
            {
                throw new ResourceRegistrationException( "Update failed: " + returnMessage.toString() );
            }
        ...
        catch ( Exception e )
        {
            ...
            if ( ResourceRegistrationException.class.isAssignableFrom( e.getClass() ) )
            {
                throw (ResourceRegistrationException) e;
            }

            throw new ResourceRegistrationException( "While registrating resource", e );
        }
    }
        ...
        throws UnknownHostException
    {
        ...
        {
            resolver.setTSIGKey( new TSIG( this.keyAlgorithm, this.keyName, this.keyData ) );
        }
        return resolver;
    }
     ...
     *
     * @param resource resource to get unregistered
        ...
        throws ResourceDeregistrationException
    {
        try
        {
            ...
            if ( DNSSDDiscoverableResource.class.isAssignableFrom( resource.getClass() ) )
            {
                ...
                if ( null == dnssdr.getPtrRecord() || null == dnssdr.getSrvRecord() || null == dnssdr.getTxtRecord() )
                {
                    throw new ResourceRegistrationException( "Resource not properly registered!" );
                }
            }
            else
            {
            ...
            Update update = new Update( domainName, DClass.IN );
            Record[] records = new Record[]{dnssdr.getPtrRecord(), dnssdr.getSrvRecord(), dnssdr.getTxtRecord()};
            update.delete( records );
            ...
            if ( 0 != returnMessage.getRcode() )
            {
                throw new ResourceDeregistrationException( "Update failed: " + returnMessage.toString() );
            }
        ...
        catch ( Exception e )
        {
            ...
            if ( ResourceDeregistrationException.class.isAssignableFrom( e.getClass() ) )
            {
                throw (ResourceDeregistrationException) e;
            }

            throw new ResourceDeregistrationException( "While registrating resource", e );
        }
    }
}
