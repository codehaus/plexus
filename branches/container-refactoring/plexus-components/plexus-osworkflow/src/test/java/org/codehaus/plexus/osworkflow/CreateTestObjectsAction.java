 ...
 * @version $Id$
 */
public class CreateTestObjectsAction
    ...
    implements Action
{
    ...
    public void execute( Map map )
        throws Exception
    {
        getLogger().info( "Creating objects." );

        TestObject object;

        List objects = new ArrayList();

        int numberOfObjects = ( (Integer) map.get( "number-of-objects" ) ).intValue();

        for( int i = 0; i < numberOfObjects; i++ )
        {
            object = new TestObject();
            object.setName( "Dude #" + i );
            objects.add( object );
        }

        map.put( "test-objects", objects );

        getLogger().info( "Created objects." );
    }
}
