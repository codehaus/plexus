 ...
 *
 * Created on November 19, 2001, 2:50 PM
 */
...
public abstract class OptionFormat {
  ...
  private static final SimpleDateFormat dateFormatYYYYMMDDHHMMSS = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
   ...
   *
   * @param description The description for this option type.
   * @param ordinal The internal index of the option type.
   */
...
/* override these methods to produce a valid, functioning formatter. -------- */
  ...
  /** Returns whether the supplied command line value is valid in accordance with
   * this option type.
   *
   ...
   * formatter.
   *
   * @param value The command line value to translate.
   ...
   * index.
   *
   * @param ordinal The internal index to lookup.
   * @return the corresponding OptionFormat instance, or null if no match found.
   */
  public static OptionFormat valueOf(int ordinal){
    if(ordinal < 0 || ordinal >= FORMATS.length){return null;}
    else{
      return FORMATS[ordinal];
    }
  }
  
  /** Lookup the OptionFormat instance corresponding to the specified description.
   *
  ...
  public static OptionFormat valueOf(String description){
    for(int i=0, len=FORMATS.length; i<len; i++){
      if(FORMATS[i].description.equals(description)){return FORMATS[i];}
    }
    return null;
  }
   ...
   * allows us to keep only one instance of a particular OptionFormat type, even
   * when the OptionFormat instance is read from a stream.
   *
  ...
  /** Validation format for command line parameters of type String.  This is really
   * pretty trivial, but provided for completeness.
   */
  ...
  /** Validation format for command line parameters of a numeric type.  Translations
   * here are to java.lang.Long.
   */
  ...
  public static final OptionFormat FRACTIONAL_NUMBER_FORMAT = new OptionFormat("Fractional Number Format"){
    public boolean isValid(String value){
      for(int i=0, len=value.length(); i<len; i++){
        ...
        if(!Character.isDigit(val) && val != '.'){return false;}
      }
      return true;
    }
  ...
  public static final OptionFormat BOOLEAN_FORMAT = new OptionFormat("Boolean Format"){
    public boolean isValid(String value){
      return ("true".equalsIgnoreCase(value) || "on".equalsIgnoreCase(value) || 
                "yes".equalsIgnoreCase(value) || "+".equalsIgnoreCase(value) ||
                "false".equalsIgnoreCase(value) || "off".equalsIgnoreCase(value) ||
                "no".equalsIgnoreCase(value) || "-".equalsIgnoreCase(value));
    }
  ...
  public static final OptionFormat DATE_FORMAT_MM_dd_yyyy = new OptionFormat("Date Format (MM/dd/yyyy)"){
    public boolean isValid(String value){
      dateFormatMMDDYYYY.format(value);
      return true;
    }
    
    public Object getValue(String value){
      return dateFormatMMDDYYYY.format(value);
    }
  };
  
  /** Validation format for date/time strings of the form yyyy-MM-dd hh:mm:ss 
   * (Standard SQL format).
   */
  public static final OptionFormat DATE_FORMAT_yyyy_MM_dd_hh_mm_ss = new OptionFormat("Date Format (yyyy-MM-dd hh:mm:ss)"){
    public boolean isValid(String value){
      dateFormatYYYYMMDDHHMMSS.format(value);
      return true;
    }
   ...
   * isValid() method returns a trivial true value, without really checking anything.
   * This is because the file may not yet exist, and yet may be a valid name.
   * The current java.io package doesn't allow for checks against the validity of
   * a filename without opening a stream and catching the IOException.
   *
  ...
  private static final OptionFormat[] FORMATS = {
    STRING_FORMAT,
    NUMBER_FORMAT,
