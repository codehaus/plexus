...
/**
 * GraphvizDirectedGraphWriter - write digraph dot files. 
 *
 * @author <a href="mailto:joakim@erdfelt.com">Joakim Erdfelt</a>
 * @version $Id$
 */
public class GraphvizDirectedGraphWriter extends AbstractGraphvizDotWriter implements GraphvizDotWriter
{
    ...
    public void writeDot( DecoratedGraph graph, File outputFile ) throws IOException, VisualizationException
    {
        if ( !( graph instanceof DirectedGraph ) )
        {
            throw new IllegalStateException( this.getClass().getName()
                            + " can only handle DecoratedGraph's that implement DirectedGraph." );
        }
        ...
        while ( it.hasNext() )
        {
            ...
            Vertex from = ( (DirectedGraph) graph ).getSource( edge );
            Vertex to = ( (DirectedGraph) graph ).getTarget( edge );
